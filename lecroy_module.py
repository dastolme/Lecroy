...
class LecroyScope():
    ...
    def action(self, action: str, directory: str = None, filename: str = None):
        if directory and filename:
            self.scope.write(f'VBS \'{action} "{directory}", "{filename}"\'')
        elif filename:
            self.scope.write(f'VBS \'{action} "{filename}"\'')
        elif directory:
            self.scope.write(f'VBS \'{action} "{directory}"\'')
        else:
    ...
    def set(self, var: str, value: VBSValue):
        self.scope.write(f'VBS \'{var} = {escape(value)}\'')
    
    def disable_auto_save(self):
        self.action('app.SaveRecall.DisableAutoSave')

    def set_auto_save_mode(self, mode: AutoSaveMode):
        if mode == AutoSaveMode.FILL:
            self.set('app.SaveRecall.AutoSave', 'FILL')
        elif mode == AutoSaveMode.WRAP:
            self.set('app.SaveRecall.AutoSave', 'WRAP')
        else:
            raise ValueError("Invalid AutoSaveMode")
        
    def set_save_directory(self, file_path: str):
        # Set the directory path for waveform save
        self.set('app.SaveRecall.Waveform.SaveTo', 'File')
        self.set('app.SaveRecall.Waveform.WaveformDir', file_path)

    def create_directory(self, file_path: str):
        # Set the directory path for waveform save
        self.action('app.SaveRecall.Utilities.DestDirectory', file_path)
        self.set('app.SaveRecall.Utilities.CreateDir', file_path)
